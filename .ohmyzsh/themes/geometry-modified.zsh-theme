...
GEOMETRY_COLOR_GIT_BRANCH=${GEOMETRY_COLOR_GIT_BRANCH:-8}
...
GEOMETRY_COLOR_SSH=${GEOMETRY_COLOR_SSH:-13}
GEOMETRY_COLOR_VENV=${GEOMETRY_COLOR_VENV:-5}
...
virtualenv_info() {
  if [ $VIRTUAL_ENV ]; then
    ...
    echo "  $(prompt_geometry_colorize $GEOMETRY_COLOR_VENV $venv)"
  fi
}

prompt_geometry_ssh_host() {
  if [[ -n $SSH_CONNECTION ]]; then
    echo "  %F{$GEOMETRY_COLOR_SSH}@%m%f"
  fi
}
...
prompt_geometry_render() {
  ...
  PROMPT="
%F{$GEOMETRY_COLOR_DIR}%3~%f $GEOMETRY_PROMPT "

  PROMPT2=" $GEOMETRY_SYMBOL_RPROMPT "
  RPROMPT="$(prompt_geometry_git_info)%f$(virtualenv_info)$(prompt_geometry_ssh_host)"
}
